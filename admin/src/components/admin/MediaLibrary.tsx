                ...
                {loading ? (
                  ...
                  <div className="table-responsive">
                    <table className="table table-sm table-hover">
                      <thead>
                        <tr>
                          <th style={{ width: '40px' }} className="align-middle text-center">
                            <input
                              type="checkbox"
                      ...
                      <tbody>
                        {filteredFiles.map((file) => (
                          <tr key={file._id} className={selectedFile === file.file_path ? 'table-active' : ''}>
                            ...
                            <td className="align-middle">{formatFileSize(file.file_size)}</td>
              ...
              <div style={{ flex: '0 0 30%', borderLeft: '1px solid #dee2e6', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                  ...
                  <div className="p-3">
                    ...
                    <div className="mb-3">
                      <label className="form-label small text-muted mb-1">Full URL</label>
                      <div className="d-flex align-items-center">
                        <p className="mb-0 small text-break flex-grow-1" style={{ wordBreak: 'break-all' }}>
                          {window.location.origin}{sidebarFile.file_path}
                        </p>
                        <button
                          type="button"
                          className="btn btn-sm btn-link text-primary p-1 ml-2"
                          onClick={() => {
                            const fullUrl = `${window.location.origin}${sidebarFile.file_path}`;
                            navigator.clipboard.writeText(fullUrl).then(() => {
                              // Show a brief success indicator
                              const btn = document.activeElement as HTMLButtonElement;
                              const originalHTML = btn.innerHTML;
                              btn.innerHTML = '<i class="fas fa-check text-success fa-xs"></i>';
                              setTimeout(() => {
