...
import '../astrologer/astrologers_call.dart';
import '../astrologer/astrologers_chat.dart';
import '../astrologer/consultations.dart';
import '../shop/orders_list.dart';
import '../wallet/wallet.dart';
import '../consultation/history.dart';
import '../support/help.dart';
import '../shop/products_list.dart';
import '../shop/product_details.dart';
import '../shop/cart.dart';
import '../consultation/chat.dart';
...
import '../consultation/incoming_chat.dart';
import '../consultation/active_call.dart';
...
class _CustomerHomeScreenState extends State<HomeScreen> {
  ...
  // Track if pending verification dialog has been shown this session
  bool _hasPendingVerificationDialogShown = false;
  ...
  Future<void> _loadData() async {
    try {
      ...
      // Check if we need to show profile complete popup
      _checkAndShowProfileCompletePopup();
    } catch (e) {
  ...
  Future<void> _loadUserData() async {
    try {
      // Refresh user data from server to get latest profile image, email, etc.
  ...
  /// Check if profile was just completed and show success popup
  Future<void> _checkAndShowProfileCompletePopup() async {
    final prefs = await SharedPreferences.getInstance();
    final shouldShow = prefs.getBool('show_profile_complete_popup') ?? false;

    if (shouldShow && mounted) {
      // Clear the flag immediately so it doesn't show again
      await prefs.remove('show_profile_complete_popup');

      // Show the popup after a small delay for smooth transition
      Future.delayed(const Duration(milliseconds: 300), () {
        if (mounted) {
          _showProfileCompletePopup();
        }
      });
    }
  }

  /// Show simple profile complete success popup
  void _showProfileCompletePopup() {
    showDialog(
      context: context,
  ...
  Future<void> _loadAstrologerDashboard() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        // Show pending verification dialog if profile is complete but not yet approved
        ...
        // Show pending verification dialog even on API failure
        _showPendingVerificationDialog();
      }
    } catch (e) {
      ...
      // Show pending verification dialog even on error
      _showPendingVerificationDialog();
    }
  }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // User Email or Phone (prefer email if available)
                    Text(
  ...
  /// Show pending verification dialog for astrologers who have completed their profile
  /// but are waiting for admin approval
  void _showPendingVerificationDialog() {
    if (_hasPendingVerificationDialogShown) return;
    if (_currentUser == null) return;
    if (!_currentUser!.isAstrologer) return;
    if (!_currentUser!.isProfileComplete) return;
    if (_currentUser!.isAdminVerified) return;

    _hasPendingVerificationDialogShown = true;
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'You can still update your profile while waiting.',
                        style: AppTextStyles.bodySmall.copyWith(
                          color: AppColors.textSecondary,
                        ),
                      ),
  ...
  /// Build rejection banner for astrologers whose profile was rejected
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Your profile needs attention. Please review and fix the issues.',
                      style: AppTextStyles.bodySmall.copyWith(
                        color: AppColors.textSecondary,
                      ),
                    ),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  _currentUser?.rejectionReason ?? 'No reason provided',
                  style: AppTextStyles.bodyMedium.copyWith(
                    color: AppColors.textPrimary,
                  ),
                ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Add your professional details to get approved and start consultations',
                      style: AppTextStyles.bodySmall.copyWith(
                        color: AppColors.textSecondary,
                      ),
                    ),
          ...
          : RefreshIndicator(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // Profile Completion Banner (for new astrologers)
                    if (_currentUser != null && !_currentUser!.isProfileComplete)
                      _buildProfileCompletionBanner(),

                    // Rejection Banner (for rejected astrologers)
                    if (_currentUser != null &&
                        _currentUser!.verificationStatus == VerificationStatus.rejected &&
                        _currentUser!.rejectionReason != null)
